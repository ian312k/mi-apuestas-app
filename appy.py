...
if 'daily_matches' not in st.session_state:
    st.session_state.daily_matches = []
...
def season_code(today=None):
    """
    Devuelve el código de temporada tipo '2526' para football-data.co.uk
    Asume cambio de temporada alrededor de julio.
    """
...
def calculate_strengths(df):
    # Protección ante promedios raros
    eps = 1e-6
    ...
    avg_home = max(avg_home, eps)
    avg_away = max(avg_away, eps)

    avg_global = (avg_home + avg_away) / 2
    avg_global = max(avg_global, eps)

    team_stats = {}
    all_teams = sorted(list(set(df['home'].unique()) | set(df['away'].unique())))
    MIX_FACTOR = 0.7  # mezcla entre home/away puro y global
...
@st.cache_data(ttl=3600)
def cached_strengths(df):
    return calculate_strengths(df)

def predict_match_dixon_coles(home, away, team_stats, avg_h, avg_a, rho=-0.13):
...
def run_backtest(df, team_stats, avg_h, avg_a, rho=-0.13):
...
def plot_gauge(val, title, color):
    # Fondo oscuro para dark mode (mejor que blanco)
    return (
        go.Figure(
            go.Indicator(
                mode="gauge+number",
...
def manage_bets(mode, data=None, id_bet=None, status=None):
    cols = ["ID", "Fecha", "Liga", "Partido", "Pick", "Cuota", "Stake", "Prob", "Estado", "Ganancia"]
    ...
    # Normalizar tipos
    for c in ["Cuota", "Stake", "Prob", "Ganancia"]:
        if c in df.columns:
...
with st.sidebar:
    ...
    # Nuevo: rho configurable
    rho = st.slider("ρ (Dixon-Coles)", -0.30, 0.10, -0.13, 0.01)

    df = fetch_live_soccer_data(code)

    # --- HISTORIAL GLOBAL EN SIDEBAR ---
    if not df.empty:
        stats, ah, aa, teams = cached_strengths(df)
    ...
    with col_ticket:
        ...
        if not st.session_state.ticket:
            ...
            for idx, item in enumerate(st.session_state.ticket):
                st.markdown(
                    f"<div class='ticket-box'><small>{item['league']}</small><br>"
                    f"<strong>{item['match']}</strong><br>{item['pick']} @ {item['odd']}</div>",
                    unsafe_allow_html=True
                )
            ...
            # (prob aprox) clamp
            total_prob = float(np.clip(total_prob, 0.0, 1.0))

            st.divider()
            st.metric("Cuota Total", f"{total_odd:.2f}")
            st.metric("Prob. Combinada (aprox)", f"{total_prob*100:.1f}%")
            ...
            if st.button("💾 Guardar"):
                ...
                manage_bets("save", {
                    ...
                    "Partido": match_str,
                    "Pick": pick_str,
                    "Cuota": round(float(total_odd), 2),
                    "Stake": float(stake_parlay),
                    "Prob": round(float(total_prob), 4),
                    "Estado": "Pendiente",
                    "Ganancia": 0.0
                })
...
with t3:
    ...
    if not db.empty:
        ...
        fig_bal.add_trace(go.Scatter(
            x=pd.to_datetime(df_plot['Fecha']),
...
with t4:
    ...
    with tab_scan_1:
        ...
        with c_add1:
            sh = st.selectbox("Local (Hoy)", teams, key="s_home")
        with c_add2:
            sa = st.selectbox("Visita (Hoy)", [t for t in teams if t != sh], key="s_away")
        with c_add3:
            st.write("")
            st.write("")
            if st.button("➕ Añadir"):
        ...
        if st.session_state.daily_matches:
            ...
            for match in st.session_state.daily_matches:
                ...
                _, _, d_ph, d_pd, d_pa, d_o15, d_o25, d_btts, _, _ = predict_match_dixon_coles(
                    mh, ma, stats, ah, aa, rho=rho
                )

                if d_ph > d_pa and d_ph > d_pd:
                    pick, prob, color = f"Gana {mh}", d_ph, "#4CAF50"
                elif d_pa > d_ph and d_pa > d_pd:
                    pick, prob, color = f"Gana {ma}", d_pa, "#2196F3"
                else:
                    pick, prob, color = "Empate", d_pd, "#FFC107"
            ...
            for _, row in df_daily.head(5).iterrows():
                st.markdown(f"""
    ...
    with tab_scan_2:
        ...
        if st.button("🚀 Escanear Liga Completa"):
            ...
            for t_h in teams:
                for t_a in teams:
                    if t_h != t_a:
                        # ✅ FIX: agarrar bien Over 2.5 (p_o25)
                        _, _, s_ph, _, s_pa, s_o15, s_o25, _, _, _ = predict_match_dixon_coles(
                            t_h, t_a, stats, ah, aa, rho=rho
                        )
                        ...
                        step += 1
                        progress.progress(step / total_scans)
...
with t5:
    ...
    if st.button("▶️ Simular 50 Partidos"):
        test_df, ok, profit = run_backtest(df, stats, ah, aa, rho=rho)
