...
def fit_ml_multiclass(X, y, seed=42):
    ...
    # Fallback: RF (no requiere xgboost)
...
# ======================================================
# 6. PESTAÑAS 📑  (AGREGUÉ SOLO 1 TAB NUEVO: 🤖 ML 1X2)
# ======================================================
...
with t7:
    ...
    if HAS_XGB:
        ...
        st.warning("⚠️ XGBoost NO instalado: usando RandomForest fallback (la app no truena).")

    # Switch rápido/estricto
    eval_mode = st.radio(
        "Modo de evaluación",
        ["⚡ Rápido (1 train + test final)", "🧪 Estricto (walk-forward sin fuga)"],
        horizontal=True,
        index=0
    )
    ...
    window_ml = cml3.slider("Ventana (matches) para snapshot/train", 300, 3000, 1200, step=100)
    ...
    if st.button("▶️ Evaluar modelo (LogLoss/Brier)"):
        ...
        if out is None:
            st.warning("No se pudo evaluar: historial insuficiente / equipos sin historial / datos faltantes.")
        else:
            m1, m2, m3, m4 = st.columns(4)
