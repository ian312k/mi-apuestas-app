...
# ======================================================
st.set_page_config(page_title="Dixon-Coles Pro v5.0 (Multi-Season)", layout="wide", page_icon="⚽")
...
# ======================================================
# 2. CARGA DE DATOS MULTI-TEMPORADA 📅
# ======================================================

@st.cache_data(ttl=3600)
def fetch_live_soccer_data(league_code="SP1"):
    """
    Descarga las últimas 3 temporadas (23/24, 24/25, 25/26),
    las une en un solo DataFrame y las ordena cronológicamente.
    """
    ...
    for i, season in enumerate(seasons):
        ...
        try:
            placeholder.text(f"Descargando temporada {season}...")
            df_season = pd.read_csv(url)
            
            # Limpieza inicial de columnas
            cols = ['Date', 'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'B365H', 'B365D', 'B365A', 'HST', 'AST']
            actual_cols = [c for c in cols if c in df_season.columns]
            df_season = df_season[actual_cols]
            
            all_dfs.append(df_season)
        except Exception:
            # Si falla (ej: temporada futura no disponible), continuamos
            continue
    ...
    # IMPORTANTE: Convertir fecha y ORDENAR para que el backtest funcione
...
# ======================================================
# 3. LÓGICA MATEMÁTICA (DIXON-COLES) 🧠
# ======================================================
...
def calculate_strengths(df):
    ...
    # Factor de olvido (0.0035 es bueno para ~3 años de datos)
    alpha = 0.0035 
    ...
    for team in all_teams:
        # Partidos donde jugó el equipo
        team_matches = df[(df['home'] == team) | (df['away'] == team)].copy()
        
        # Stats Globales Ponderadas
...
# ======================================================
# 4. BACKTEST BLINDADO (SIN FUGAS) 🛡️
# ======================================================
def run_backtest_blindado(full_df, num_matches=40):
    """
    Simulación Walk-Forward:
    Para cada partido, usa SOLO datos anteriores para recalcular el modelo.
    """
    # Ordenar y limpiar
    ...
    # Seleccionar últimos N partidos
    start_index = max(100, total_len - num_matches)
    ...
    for i, idx in enumerate(test_indices):
        ...
        progress_bar.progress(pct, text=f"Analizando partido {i+1}/{len(test_indices)}...")
        
        row = df.iloc[idx]
        match_date = row['date']
        home_team, away_team = row['home'], row['away']
        
        # 1. CORTE TEMPORAL (Viaje en el tiempo)
        ...
        # 4. BUSCAR VALOR (EV)
        # Solo apostamos si EV > 5% (Margen de seguridad)
        ev_h = (ph * row['odd_h']) - 1
        ...
        if best_ev > THRESHOLD:
            ...
            if best_ev == ev_h:
                pick = "Local"; used_odd = row['odd_h']; used_prob = ph
                profit = (used_odd - 1) * stake if res_real == "Local" else -stake
            elif best_ev == ev_a:
                pick = "Visita"; used_odd = row['odd_a']; used_prob = pa
                profit = (used_odd - 1) * stake if res_real == "Visita" else -stake
            else:
                pick = "Empate"; used_odd = row['odd_d']; used_prob = pd_prob
                profit = (used_odd - 1) * stake if res_real == "Empate" else -stake
            ...
            results.append({
                ...
                "Pick": pick,
                "Cuota": round(used_odd, 2),
                "Prob": f"{used_prob*100:.1f}%",
                "EV": f"{best_ev*100:.1f}%",
                "Res": res_real,
                "P/L": round(profit, 2)
            })
...
with st.sidebar:
    ...
    # Carga de datos multi-temporada
    df = fetch_live_soccer_data(code)
    
    if not df.empty:
        # Cálculo de fuerzas actuales (para predecir mañana)
        stats_current, ah, aa, teams = calculate_strengths(df)
        st.success(f"✅ {len(df)} partidos cargados (3 Temp)")
    ...
    else: st.error("No se pudieron cargar datos."); st.stop()
...
with t1:
    ...
    fig_shot.update_layout(barmode='group', title="Realidad vs Modelo (Tiros)", height=300, margin=dict(t=30, b=20))
...
with t4:
    ...
    api_key_input = st.text_input("🔑 API Key:", value=st.session_state.api_key, type="password", key="api_input_key")
    if api_key_input != st.session_state.api_key: st.session_state.api_key = api_key_input; st.rerun()
    
    if st.button("⬇️ Descargar Datos (The-Odds-API)"):
    ...
    if code in st.session_state.market_storage:
        ...
        st.info("Datos cargados en memoria. Ve a la pestaña 'Valor' para ver comparativas automáticas.")

# --- TAB 5: LABORATORIO BLINDADO ---
with t5:
    st.markdown("## 🧪 Laboratorio Anti-Fugas")
    st.info("Este simulador viaja al pasado y recalcula el modelo partido a partido para evitar el 'Look-ahead Bias'.")
    
    col_sim1, col_sim2 = st.columns(2)
    # Permitimos hasta 200 partidos porque ahora tenemos 3 temporadas
    games_to_sim = col_sim1.slider("Partidos a simular (hacia atrás)", 20, 200, 50)
    
    if st.button("▶️ Ejecutar Validación Walk-Forward"):
        if len(df) < 100:
            st.error("Necesitas más datos históricos para ejecutar esto.")
        else:
            with st.spinner("Procesando simulación..."):
                ...
                if not df_res.empty:
                    ...
                    m3.metric("Beneficio (U)", f"{bal:.2f} u", delta_color="normal")
                    ...
                    st.warning("No se encontraron apuestas con valor en el periodo seleccionado.")

# --- TAB 6: BI ---
with t6:
    st.markdown("## 📈 Rendimiento Real (Tus apuestas)")
